...
- **教具舞台（后端权威 + SSE 实时下发）：** 展示哪张图由后端 `PropStageDirector` 按外教本轮最终文本判定，每轮通过 SSE `turn.prop` 事件下发；前端只渲染，不做任何字幕正则、颜色线索或主题特判。外教点名了本课已发布道具 → **左右分屏**（`propFocus`，约 38%/62%，最多 3 张，活跃图为第一个）；没点名 → 人像主视觉（`personaFocus`）。`blocked` 一律收起分屏。开课返回的 `session.propAssets` 只用于预热图片缓存，不决定首帧布局。取图失败退 `/props/default/star.svg` 占位。只读图 `GET /api/cet/props/{lemma}`（JWT，带 ETag/304）。见 [2026-09-14-cet-prop-realtime-stage-design.md](superpowers/specs/2026-09-14-cet-prop-realtime-stage-design.md)
