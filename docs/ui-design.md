...
- **播放已起播后的 `error` 视同播完：** 厂商 mp3 末帧不完整时 Chrome 只抛 `MEDIA_ERR_DECODE`、不再触发 `ended`；此时走与 `ended` 相同的补齐静默路径，不得立刻抢麦（末帧本身在 `kidora-mcp-server` 侧修补，见 [mcp-design.md](mcp-design.md) §10.1）
