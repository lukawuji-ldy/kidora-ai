...
/**
 * Security 未认证 / 无权限时返回统一 {@code ApiResponse} JSON，避免空 body 导致前端 {@code res.json()} 失败。
 *
 * @author liudy
 */
public final class JsonSecurityErrorWriter {
    ...
    public static ServerAccessDeniedHandler accessDeniedHandler() {
        return (ServerWebExchange exchange, org.springframework.security.access.AccessDeniedException denied) ->
                write(exchange.getResponse(), HttpStatus.FORBIDDEN, ErrorCode.FORBIDDEN);
    }
