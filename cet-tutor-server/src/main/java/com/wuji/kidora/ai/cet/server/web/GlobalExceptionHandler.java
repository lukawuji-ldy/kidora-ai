...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleKidora(KidoraException ex) {
        ...
        if (status.is5xxServerError()) {
            log.error("CET KidoraException {} : {}", ex.getErrorCode(), ex.getMessage(), ex);
        } else {
            log.warn("CET KidoraException {} : {}", ex.getErrorCode(), ex.getMessage());
        }
    ...
    public ResponseEntity<ApiResponse<Void>> handleOther(Exception ex) {
        log.error("CET unhandled exception → INTERNAL_ERROR", ex);
